...
class NaiveBayesClassifier:
    ...
    def classify(self, features):
        ...
        for c in self.categories:
            over = self.categories[c] + self.alpha
            under = self.training_count + len(self.categories) * self.alpha
            score = float(over / under)
            for f in features:
                if f not in self.features.keys():
                    score *= self.alpha / (self.categories[c] + 2 * self.alpha)
                else:
                    over = self.features[f][c] + self.alpha
                    under = self.categories[c] + 2 * self.alpha
                    score *= (over / under)
            if max_score < score:
